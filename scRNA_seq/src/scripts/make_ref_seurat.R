...
thymic_reference_seurat <- CreateSeuratObject(counts = exprs)

thymic_reference_seurat <- AddMetadata(colData(thymic_reference_sce))

# thymic_reference_seurat <- as.Seurat(thymic_reference_sce, counts = "logcounts",
#  	data = "logcounts")

saveRDS(thymic_reference_seurat, "files/thymus_annotated_matrix_files/seurat_object.rda")